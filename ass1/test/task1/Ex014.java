...
public class Ex014
{
  ...
  public static void main(String[] args)
  {
    ...
    locks.put("Lock3", new LockTreeLock("Lock3"));
    locks.put("Lock4", new LockTreeLock("Lock4"));
    ...
    thread1.addAction(new ThreadAction("LOCK","Lock3"));
    thread1.addAction(new ThreadAction("LOCK","Lock4"));
    thread1.addAction(new ThreadAction("UNLOCK","Lock4"));
    thread1.addAction(new ThreadAction("UNLOCK","Lock3"));

    final MyThread thread2 = new MyThread("ExampleThread 2", locks);
    thread2.addAction(new ThreadAction("LOCK","Lock4"));  
    thread2.addAction(new ThreadAction("LOCK","Lock3"));  
    thread2.addAction(new ThreadAction("UNLOCK","Lock3"));
    thread2.addAction(new ThreadAction("UNLOCK","Lock4"));
        
    final Ex014 deadlockExample = new Ex014();
    deadlockExample.addThread(thread1);
    deadlockExample.addThread(thread2);
  ...
  private static class MyThread extends Thread
  {
    ...
    {
      if(!locks.keySet().contains(action.getLockName())) throw new RuntimeException("no lock created for name '" + action.getLockName()+ "'");
